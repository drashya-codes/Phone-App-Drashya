	...
	zim.Physics = function(gravity, borders, scroll, frame) {
		...
		this.Drag = function(list) {
			...
			this.removeListeners = function() {
				isMouseDown = false;
				if (that.mousedownEvent) stage.off("stagemousedown", that.mousedownEvent);
				if (that.mousemoveEvent) stage.off("stagemousemove", that.mousemoveEvent);
				if (that.mouseupEvent) stage.off("stagemouseup", that.mouseupEvent);
			...
			that.mouseupEvent = stage.on("stagemouseup", function(e) {
				isMouseDown = false;
				stage.off("stagemousemove", that.mousemoveEvent);
				mouseX = (e.stageX-(scroll?stage.x:0))/scale;
				mouseY = (e.stageY-(scroll?stage.y:0))/scale;
			});
